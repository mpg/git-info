\documentclass{minimal}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}
\usepackage{ted, trace}
\usepackage[
maxdepth=0,
% helper-file=nosuchfile.ext
]{git-info}

\makeatletter
\newcommand\testdate[3][+0000]{%
  \begingroup
  \def\GI@head@timestamp{#2}%
  \def\GI@head@timezone{#1}%
  \GI@get@gen@datetime{head}%
  \message{^^J#2#1:^^JMacro: \githeaddatetime}%
  \message{^^JReal: \space#3}%
  \endgroup}
\makeatother

% \testdate{0}{1970-01-01T00:00:00+00:00}
% \testdate{2865906}{1970-02-03T04:05:06+00:00}
% \testdate{31535999}{1970-12-31T23:59:59+00:00}
% \testdate{31536000}{1971-01-01T00:00:00+00:00}
% \testdate{951955200}{2000-03-02T00:00:00+00:00}
% \testdate{951868799}{2000-02-29T23:59:59+00:00}
% \testdate{951868800}{2000-03-01T00:00:00+00:00}
% \testdate{978307199}{2000-12-31T23:59:59+00:00}
% \testdate{978307200}{2001-01-01T00:00:00+00:00}
% \testdate{983404799}{2001-02-28T23:59:59+00:00}
% \testdate{983404800}{2001-03-01T00:00:00+00:00}

\newcommand\showval[2]{%
  \message{^^J#2: -=|#1|=-^^J}}

% \showval{\gitinfo{@head}{commit}}{Git commit}
% \showval{\gitinfo{@head}{branch}}{Git branch}
% \showval{\gitinfo{@head}{author}}{Git author}
% \showval{\gitinfo{@head}{mail}}{Git mail}
% \showval{\gitinfo{@head}{datetime}}{Git head datetime}

% \showval\githeaddate{Git head date}
% \showval\githeadtime{Git head time}
% \showval\githeadtimezone{Git head timezone}
% \showval\githeadyear{Git head year}
% \showval\githeadmonth{Git head month}
% \showval\githeadday{Git head day}
% \showval\githeadhour{Git head hour}
% \showval\githeadmin{Git head min}
% \showval\githeadsec{Git head sec}
% \showval\githeadtzsign{Git head tzsign}
% \showval\githeadtzhour{Git head tzhour}
% \showval\githeadtzmin{Git head tzmin}
% \newcommand\myformat[9]{#1,#2,#3,#4,#5,#6,#7,#8,#9}
% \showval{\githeaddtformat\myformat}{Git head dtformat}

% package options
% \makeatletter
% \showval\GI@max@depth{Internal value 'maxdepth'}
% \showval\GI@gih@name{Internal value 'gihname'}
% \makeatother

\begin{document}

% Not expandable, so must be tested in typesetting context only.
% Githeadtoday: -=|\githeadtoday|=-\par

% Testing error messages (not expandable)
\gitinfo{@head}{datetime}

\end{document}
